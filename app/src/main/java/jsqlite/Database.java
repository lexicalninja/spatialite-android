...
public class Database {
	...
	if ((mode & 0200) != 0) {
	    mode = Constants.SQLITE_OPEN_READWRITE |
		   Constants.SQLITE_OPEN_CREATE;
	} else if ((mode & 0400) != 0) {
	    mode = Constants.SQLITE_OPEN_READONLY;
	}
	synchronized(this) {
	    try {
    ...
    private native void _exec(String sql, Callback cb)
	throws Exception;
    ...
    private native void _exec(String sql, Callback cb, String args[])
	throws Exception;
    ...
    private native void _busy_handler(BusyHandler bh);
    ...
    public Blob open_blob(String db, String table, String column,
			  long row, boolean rw) throws Exception {
	synchronized(this) {
	    Blob blob = new Blob();
    ...
    private native void _progress_handler(int n, ProgressHandler p);
	...
	try {
	    ...
	    throw new Exception("not a julian date: " + s + ": " + ee);
	}
    }
